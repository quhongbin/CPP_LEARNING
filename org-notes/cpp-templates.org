...
Another critical Godot template is ~Ref<T>~ ([[id:20250527T230614][C++ Smart Pointers & Ref]]), which is a reference-counted smart pointer template. It only accepts classes derived from ~RefCounted~, enforced through template constraints.
